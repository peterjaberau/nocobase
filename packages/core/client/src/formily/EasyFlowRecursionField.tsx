...
import { EasyFlowField } from './EasyFlowField';

interface IEasyFlowRecursionFieldProps extends IRecursionFieldProps {
  /**
    ...
    <Fragment>
      {properties.map(({ schema: item, key: name }, index) => {
        ...
        if (schema['x-component'] === 'CollectionField') {
          return (
            <IsInEasyFlowRecursionFieldContext.Provider value={true} key={`${index}-${name}`}>
              <CollectionFieldUISchemaProvider fieldSchema={schema}>{content}</CollectionFieldUISchemaProvider>
            </IsInEasyFlowRecursionFieldContext.Provider>
          );
        }

        return (
          <IsInEasyFlowRecursionFieldContext.Provider value={false} key={`${index}-${name}`}>
...
export const IsInEasyFlowRecursionFieldContext = React.createContext(false);

/**
 * @internal
 * Note: Only suitable for use within the CollectionField component
 */
export const useIsInEasyFlowRecursionFieldContext = () => {
  return React.useContext(IsInEasyFlowRecursionFieldContext);
};

/**
 * Based on @formily/react v2.3.2 RecursionField component
 * Modified to better adapt to EasyFlow's needs
 */
export const EasyFlowRecursionField: ReactFC<IEasyFlowRecursionFieldProps> = React.memo((props) => {
  const {
    schema,
    name,
  ...
  const render = () => {
    ...
    return isUseFormilyField ? (
      ...
      <EasyFlowField name={name} value={values} initialValue={values} basePath={basePath} schema={mergedFieldSchema} />
    );
  };

  if (!fieldSchema) return <Fragment />;

  // The original fieldSchema is still passed down to maintain compatibility with EasyFlow usage.
...
EasyFlowRecursionField.displayName = 'EasyFlowRecursionField';
