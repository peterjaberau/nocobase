...
export function updateActionContextAction(context: ActionContextType): BroadcastAction<UpdateActionContextAction> {
...
export function multiChangeAction(changes: Record<string, CompAction>): MultiChangeAction {
  ...
  console.assert(
    Object.values(changes).every((action) => !_.isNil(action.editDSL) && action.editDSL === editDSL),
...
export function changeChildAction(childName: string, value: JSONValue, editDSL: boolean): CompAction {
...
export function wrapActionExtraInfo<T extends CompAction>(action: T, extraInfos: ActionExtraInfo): T {
