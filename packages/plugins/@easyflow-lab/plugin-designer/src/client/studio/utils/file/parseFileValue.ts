...
export const handleParseSheets = (data: any, type: 'base64' | 'buffer' | 'string' = 'base64') => {
  let res = {};
...
export const handleParseImage = async (file: File, needUploadImage?: boolean): Promise<string> => {
  if (needUploadImage) {
    // upload image
    return 'image url';
  }
...
export const handleParseFile = async (file: File, needUploadImage?: boolean): Promise<string> => {
