...
import { isArray, isObject } from '../system/is';
...
export const downloadSingleFile = (contentType: string, fileName: string, data: BlobPart) => {
...
export const getContentTypeByFileExtension = (extension: string) => {
  return (
    FILE_EXTENSION_TO_CONTENT_TYPE_MAP[(extension ?? '').toLowerCase() as keyof typeof FILE_EXTENSION_TO_CONTENT_TYPE_MAP] ||
    'application/octet-stream'
  );
};

export const downloadExcelFile = (data: any, contentType: string) => {
  const bookType = contentType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' ? 'xlsx' : 'xls';
...
export const getFileName = (fileName: string, fileType: string) => {
  ...
  const fileRealName = (!fileName ? 'download' : length > 1 ? namePart.slice(0, -1).join('.') : namePart[0]) || 'download';
...
export const downloadFileFromURL = async (data: string, fileDownloadName: string, contentType: string) => {
