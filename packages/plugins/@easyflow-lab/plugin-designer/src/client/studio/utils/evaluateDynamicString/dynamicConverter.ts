...
export const getSnippets = (dynamicString: string): { stringSnippets: string[]; jsSnippets: string[] } => {
  ...
  const stringSnippets = getStringSnippets(sanitisedString);
  const jsSnippets = stringSnippets.map((segment) => {
...
export const getDynamicValue = (dynamicString: string, dataTree: Record<string, any>) => {
