import { CouchDBActionStructParamsDataTransferType, DynamoActionStructParamsDataTransferType, GoogleSheetDataTypeTransform, HuggingFaceBooleanTypes, HuggingFaceBooleanValueMap } from '../../../public-components/public-config';
import {
  ActionType,
  AirtableAction,
...
} from '../../../public-components/public-types';
import { Params } from '../../../public-components/public-types/public';
import { isNumber, isString, isObject } from '../system/is';
...
export const transformDataFormat = (actionType: ActionType, contents: Record<string, any>) => {
  switch (actionType) {
    case 'smtp': {
    ...
    case 'firebase': {
      ...
      if (service === FirebaseServiceTypeValue.AUTH && operation === FirebaseAuthActionTypeValue.LIST_USERS) {
      ...
      if (service === FirebaseServiceTypeValue.FIRESTORE && (operation === FirebaseStoreActionTypeValue.QUERY_FIREBASE || operation === FirebaseStoreActionTypeValue.QUERY_COLLECTION_GROUP)) {
    ...
    case 'hfendpoint': {
      ...
      const realDetailParams = keys.map((key: string) => {
        ...
        return {
          key,
          value: currentValue ? (HuggingFaceBooleanTypes.includes(key) ? HuggingFaceBooleanValueMap[currentValue as keyof typeof HuggingFaceBooleanValueMap] ?? currentValue : parseFloat(currentValue)) : '',
        };
      });
      return {
        ...(!isEndpoint && { modelID }),
    ...
    case 'dynamodb': {
      ...
      const newStructParams = { ...structParams };
    ...
    case 'googlesheets': {
      ...
      const googleSheetsTransformKeys = Object.keys(GoogleSheetDataTypeTransform);
      const newGoogleSheetOpts = { ...googleOpts };
      googleSheetsTransformKeys.forEach((key) => {
        const value = GoogleSheetDataTypeTransform[key as keyof typeof GoogleSheetDataTypeTransform];
      ...
      switch (method) {
        case 'list': {
          ...
          return {
            ...
            config: {
              ...
              maxRecords: listConfig.maxRecords === '' ? -1 : listConfig.maxRecords,
        ...
        case 'delete': {
          ...
          return {
            ...
            config: {
              ...deleteConfig,
              recordID: deleteConfig.recordID === '' ? '' : deleteConfig.recordID,
            },
          };
        }
        case 'create': {
        ...
        case 'bulkUpdate': {
          const bulkUpdateConfig = contents.config as AirtableUpdateMultipleRecords;
          return {
            ...contents,
            config: {
              ...bulkUpdateConfig,
              records: bulkUpdateConfig.records === '' ? [] : bulkUpdateConfig.records,
            },
          };
        }
        case 'bulkDelete': {
          const bulkDeleteConfig = contents.config as AirtableDeleteMultipleRecords;
          return {
            ...contents,
            config: {
              ...bulkDeleteConfig,
              recordIDs: bulkDeleteConfig.recordIDs === '' ? [] : bulkDeleteConfig.recordIDs,
            },
          };
        }
        default:
    ...
    case 'aiagent': {
      return {
        ...
        modelConfig: {
          maxTokens: (contents.virtualResource?.modelConfig ?? contents.modelConfig).maxTokens,
          stream: false,
        },
    ...
    // case 'illadrive': {
    //   return transformDriveData(contents as ILLADriveAction<ILLADriveActionTypeContent>);
    // }
    default:
      return contents;
  }
