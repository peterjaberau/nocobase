import { getILLABuilderURL, getILLACloudURL, removeAuthToken } from '../../../utils';
...
export const errorHandlerInterceptor = (error: AxiosError) => {
  ...
  switch (status) {
    case 401: {
      removeAuthToken();
      window.location.href = `${getILLACloudURL(window.customDomain)}?redirectURL=${encodeURIComponent(
