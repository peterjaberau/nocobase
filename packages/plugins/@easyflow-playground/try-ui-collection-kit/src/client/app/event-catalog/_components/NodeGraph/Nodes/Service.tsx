    ...
    <ContextMenu.Root>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content className="min-w-[220px] bg-white rounded-md p-1 shadow-md border border-gray-200">
          ...
          >
            {/*buildUrl(`/docs/services/${id}/${version}`)*/}
          ...
          {asyncApiFiles.length > 0 &&
            asyncApiFiles.map((file: any) => (
              <ContextMenu.Item asChild key={file.path}>
                {/*buildUrl(`/docs/services/${id}/${version}/asyncapi/${file.filename}`)*/}
                <a
                  href={''}
                  className="text-sm px-2 py-1.5 outline-none cursor-pointer hover:bg-orange-100 rounded-sm flex items-center"
                  target="_blank"
          ...
          {openApiFiles.length > 0 &&
            openApiFiles.map((file: any) => (
              <ContextMenu.Item asChild key={file.path}>
                {/*buildUrl(`/docs/services/${id}/${version}/spec/${file.filename}`)*/}
                <a
                  href={''}
                  className="text-sm px-2 py-1.5 outline-none cursor-pointer hover:bg-orange-100 rounded-sm flex items-center"
                  target="_blank"
          ...
          <ContextMenu.Item asChild>
            {/*buildUrl(`/docs/services/${id}/${version}/changelog`)*/}
            <a
              href={''}
              className="text-sm px-2 py-1.5 outline-none cursor-pointer hover:bg-orange-100 rounded-sm flex items-center"
              target="_blank"
