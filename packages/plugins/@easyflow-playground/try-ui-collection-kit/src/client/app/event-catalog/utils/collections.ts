...
export const getVersions = (data: any[]) => {
...
export const getVersionForCollectionItem = (item: any, collection: any[]) => {
...
export const findMatchingNodes = (
  // node type in collections events' | 'commands' | 'queries' | 'services
  nodesA: any[],
  nodesB: any[],
) => {
...
export const getDeprecatedDetails = (item: any) => {
  let options = {
  ...
  if (typeof item.data.deprecated === 'object') {
    ...
    options.deprecatedDate = item.data.deprecated.date
      ? new Date(item.data.deprecated.date).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        })
